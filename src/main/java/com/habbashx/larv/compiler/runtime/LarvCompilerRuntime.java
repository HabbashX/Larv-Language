...
public class LarvCompilerRuntime {
    ...
    private static final java.util.concurrent.ExecutorService ASYNC_EXECUTOR =
            java.util.concurrent.Executors.newCachedThreadPool(r -> {
                Thread t = new Thread(r, "larv-async");
                t.setDaemon(true);
                return t;
            });
    ...
    /**
     * Runs the compiled {@code async} body method on the shared async executor
     * and immediately returns a {@link java.util.concurrent.CompletableFuture}.
     * The wrapper generated by the compiler passes the owner class (as a class
     * constant, so it resolves through the Larv class loader), an optional
     * receiver instance, the {@code <name>$async$body} method name and the
     * boxed argument array.
     */
    public static Object runAsync(Class<?> bodyClass, Object target, String methodName, Object[] args) {
        return java.util.concurrent.CompletableFuture.supplyAsync(
                () -> invokeAsyncBody(bodyClass, target, methodName, args), ASYNC_EXECUTOR);
    }

    private static Object invokeAsyncBody(Class<?> cls, Object target, String methodName, Object[] args) {
        try {
            Method m = resolveMethod(cls, methodName, args.length);
            if (m == null) {
                throw new LarvRuntimeException("async: body method not found: " + cls.getName() + "." + methodName);
            }
            try {
            ...
            throw new LarvRuntimeException("async task failed: " + e.getMessage(), e);
        }
    }

    /**
     * Unwraps a future/completion-stage produced by an {@code async} function,
     * blocking until it completes.  Non-future values pass through unchanged,
     * so {@code await} is safe on plain expressions too.
     */
    public static Object awaitValue(Object value) {
        if (value instanceof java.util.concurrent.Future<?> future) {
            try {
                ...
                throw new LarvRuntimeException("await interrupted: " + e.getMessage(), e);
            } catch (java.util.concurrent.ExecutionException e) {
                Throwable cause = e.getCause();
                if (cause instanceof RuntimeException re) throw re;
                if (cause instanceof Error err) throw err;
                throw new LarvRuntimeException("await failed: " + (cause != null ? cause.getMessage() : e.getMessage()), cause);
            }
        }
        return value;
    }
    ...
    /**
     * Postfix non-null assertion ({@code expr!}): returns the value unchanged
     * if non-null, otherwise throws.
     */
    @Contract("null -> fail")
    public static Object requireNonNullValue(Object value) {
        if (value == null) throw new LarvRuntimeException("Non-null assertion failed: value was nil.");
        return value;
    }
        ...
        for (int i = 0; i < stringArgs.length; i++) {
            ...
            if (staticField != null) {
                if (targetType.isInstance(staticField) || targetType == Object.class) {
                    mapped[i] = staticField;
                    continue;
                }
                if (targetType == String.class) {
                    // The raw arg denotes a static field of a different type (e.g. System.in);
                    // reject this ctor so the field-accepting one is chosen.
                    return null;
                }
            }
    ...
    private static @Nullable Object tryResolveStaticField(String rawArg) {
        rawArg = rawArg.trim();
        ...
        try {
            Class<?> cls = tryLoadClass(className);
            if (cls == null) cls = tryLoadClass("java.lang." + className);
    ...
    private static @Nullable Class<?> tryLoadClass(String name) {
